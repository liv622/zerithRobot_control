...
class CoreTests(unittest.TestCase):
    ...
    def test_tcp_is_fixed_extension_from_link7(self) -> None:
        arm = self.model.arm_vector(INITIAL_CONFIGURATION)
        aux = self.model.aux_configuration(INITIAL_CONFIGURATION)
        flange = self.model.flange_pose(arm, aux)
        tcp = self.model.tcp_pose(arm, aux)
        local_offset = flange[:3, :3].T @ (tcp[:3, 3] - flange[:3, 3])
        np.testing.assert_allclose(
            local_offset, [0, 0, TCP_EXTENSION_M], atol=1e-12
        )
    ...
    def test_ik_preserves_auxiliary_mechanisms_and_writes_result(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(set(saved["arm_joints_rad"]), set(ARM_JOINTS))
            self.assertEqual(set(saved["auxiliary_joints"]), set(AUX_JOINTS))
